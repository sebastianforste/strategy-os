...
export async function generateContent(
  ...
  const strategicContext = await fetchContext(input);
  if (strategicContext) {
      console.log("[AI Service] Injecting Strategic Context...");
      systemInstruction += "\n" + strategicContext;
  }
...
export async function generateComment(
  ...
  options: {
    ...
    const genAI = new GoogleGenAI({ apiKey });
    const modelName = process.env.NEXT_PUBLIC_GEMINI_FALLBACK_MODEL || "models/gemini-3-flash-preview";
