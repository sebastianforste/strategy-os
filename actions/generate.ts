...
export async function scoreViralityAction(content: string, apiKey: string, personaId: string = "cso") {
    if (!apiKey) throw new Error("API Key required");
    const { getViralityPrediction } = await import("../utils/analytics-service");
    const prediction = await getViralityPrediction(content, personaId, apiKey);
    
    // Map to the format expected by the UI (PredictiveResult)
    return {
        score: prediction.score,
        breakdown: {
            hookStrength: prediction.breakdown.hook,
            retainability: prediction.breakdown.formatting,
            viralityPotential: prediction.breakdown.strategy
        },
        critique: prediction.critique,
        improvementTips: prediction.hooks
    };
}
...
export async function evolvePersonaAction(
    ...
    // 4. Persist (Phase 83: Auto-Prompt Optimization)
